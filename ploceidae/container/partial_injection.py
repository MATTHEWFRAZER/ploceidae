...
class PartialInjection(object):
    ...
    def get_dependencies_to_be_injected(self, dependencies_to_be_injected):
        ...
        dependencies = filter(lambda x: x not in BINDINGS, get_dependencies(self.dependent_obj))
        return [self.dependencies_to_be_injected[dependency] for dependency in dependencies] + list(self.grouped_dependencies_to_be_injected)